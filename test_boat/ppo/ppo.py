...
class PPO(PPO_Base):
    def _build_net(self, s_inputs, scope, trainable, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            mu = tf.layers.dense(mu, 256, tf.nn.relu, trainable=trainable, **initializer_helper)
            ...
            sigma = tf.layers.dense(sigma, 256, tf.nn.relu, trainable=trainable, **initializer_helper)
