...
class PPO(PPO_Base):
    ...
    def _build_actor_net(self, s_inputs, scope, trainable, reuse=False):
        with tf.variable_scope(scope, reuse=reuse):
            ...
            mu = tf.layers.dense(mu, 128, tf.nn.relu, trainable=trainable, **initializer_helper)
            mu = tf.layers.dense(mu, 128, tf.nn.relu, trainable=trainable, **initializer_helper)
            ...
            sigma = tf.layers.dense(sigma, 128, tf.nn.relu, trainable=trainable, **initializer_helper)
            sigma = tf.layers.dense(sigma, 128, tf.nn.relu, trainable=trainable, **initializer_helper)
