...
with tf.Session() as sess:
    ...
    saver = Saver('model/model_sep_nn', sess)
    last_iteration = saver.restore_or_init()

    if train_mode:
        time_str = time.strftime('%Y%m%d%H%M%S', time.localtime(time.time()))
        summary_writer = tf.summary.FileWriter(f'log/sep_nn', sess.graph)
        summary_writer.close()
        summary_writer = tf.summary.FileWriter(f'log/sep_nn/{time_str}')
