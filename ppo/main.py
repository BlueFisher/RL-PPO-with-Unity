...
def simulate_multippo(env, brain_info, default_brain_name, action_dim, ppos: list):
    ...
    not_hitted_trans_discounted_all = [[] for _ in range(len_agents)]
        ...
        for i in range(len_agents):
            ...
            for tran in trans[::-1]:  # state, action, reward, done, max_reached, state_
                ...
                if is_hitted_tran:
                    ...
                    not_hitted_trans_discounted_all[i].append(tran)

            hitted_trans_discounted_all[i].reverse()
            not_hitted_trans_discounted_all[i].reverse()

        return brain_info, trans_discounted_all, rewards_all, hitted_all, hitted_real_all, hitted_trans_discounted_all, not_hitted_trans_discounted_all
    else:
        return brain_info, None, rewards_all, hitted_all, hitted_real_all, None, None
...
for iteration in range(ITER_MAX + 1):
    ...
    brain_info, trans_discounted_all, rewards_all, hitted_all, hitted_real_all, \
        hitted_trans_discounted_all, not_hitted_trans_discounted_all = \
    ...
    for i, ppo in enumerate(ppos):
        ...
        if TRAIN_MODE:
            ...
            if len(not_self_hitted_trans_discounted) > 0:
                ...
                not_self_hitted_trans_discounted_true = [not_self_hitted_trans_discounted[i] for i, v in enumerate(bool_mask) if v]

            trans_discounted += not_self_hitted_trans_discounted_true
