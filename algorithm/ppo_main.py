...
class Main(object):
    ...
    def __init__(self, argv, agent_class=Agent):
        ...
        self.config, self.reset_config, ppo_config, model_root_path = self._init_config(argv)
        self._init_env(ppo_config, model_root_path)
        self._run()
    ...
    def _init_config(self, argv):
        ...
        return config, reset_config, ppo_config, model_root_path

    def _init_env(self, ppo_config, model_root_path):
        config = self.config
        ...
        for i in range(config['policies_num']):
            ...
            if config['std']:
                class PPO(importlib.import_module(config['ppo']).PPO_Std_Custom, PPO_Base):
                    pass
            else:
                class PPO(importlib.import_module(config['ppo']).PPO_Sep_Custom, PPO_Base):
                    pass

            logger.info(tmp_model_root_path)
            self.ppos.append(PPO(state_dim=state_dim,
