...
class PPO(PPO_SEP_NN):
    ...
    def _build_actor_net(self, s_inputs, scope, trainable):
        with tf.variable_scope(scope):
            ...
            mu = tf.layers.dense(l, self.a_dim, tf.nn.tanh, trainable=trainable, **initializer_helper)
            sigma = tf.layers.dense(l, self.a_dim, tf.nn.sigmoid, trainable=trainable, **initializer_helper)
