...
class PPO(PPO_Base):
    def _build_net(self, s_inputs, scope, trainable):
        with tf.variable_scope(scope):
            ...
            prob_l = tf.layers.dense(l, 512, tf.nn.relu, trainable=trainable, **initializer_helper)
            mu = tf.layers.dense(prob_l, 512, tf.nn.relu, trainable=trainable, **initializer_helper)
            mu = tf.layers.dense(mu, self.a_dim, tf.nn.tanh, trainable=trainable, **initializer_helper)
            sigma = tf.layers.dense(prob_l, 512, tf.nn.relu, trainable=trainable, **initializer_helper)
            ...
            v_l = tf.layers.dense(l, 512, tf.nn.relu, trainable=trainable, **initializer_helper)
            v_l = tf.layers.dense(v_l, 512, tf.nn.relu, trainable=trainable, **initializer_helper)
