...
with tf.Session() as sess:
    ...
    for episode in range(ITER_MAX):
        ...
        while True not in dones_control and steps_n < MAX_STEPS:
            actions = {}
            if train_mode:
            ...
            rewards_list[attack_brain_name].append(rewards[attack_brain_name])
            rewards_list[guard_brain_name].append(rewards[guard_brain_name])
        ...
        print(f'a: rewards {sum(rewards_list[attack_brain_name]):.2f}\t', end='')
        if train_mode:
            print(f'var {attack_agent.variance:.3f}')

        print(f'g: rewards {sum(rewards_list[guard_brain_name]):.2f}\t', end='')
        if train_mode:
            print(f'var {guard_agent.variance:.3f}')

        if train_mode:
            summaries = tf.Summary(value=[
                tf.Summary.Value(tag='reward/attack', simple_value=sum(rewards_list[attack_brain_name])),
                tf.Summary.Value(tag='reward/guard', simple_value=sum(rewards_list[guard_brain_name]))
            ])
